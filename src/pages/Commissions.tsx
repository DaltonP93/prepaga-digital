...
export default function Commissions() {
  ...
  if (settings.isError || !settings.data?.is_enabled) return <div className="mx-auto max-w-3xl py-10"><Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Módulo no habilitado</AlertTitle><AlertDescription>La configuración de comisiones no existe, no está activa o no pudo validarse. Por seguridad, el acceso permanece cerrado.</AlertDescription></Alert></div>;
  return <div className="space-y-6"><div><h1 className="text-3xl font-bold tracking-tight">Comisiones</h1><p className="text-muted-foreground">Configuración, cálculo y liquidación de comisiones comerciales.</p></div>
    <Tabs defaultValue="periods"><TabsList><TabsTrigger value="periods">Liquidaciones</TabsTrigger>{canManage && <TabsTrigger value="preview">Nueva liquidación</TabsTrigger>}{canReadRules && <TabsTrigger value="rules">Reglas</TabsTrigger>}</TabsList>
