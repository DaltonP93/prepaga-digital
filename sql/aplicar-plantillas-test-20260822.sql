-- =====================================================================
-- Plantillas de los 3 tipos de venta — aplicar en TEST (US)
--
--   Proyecto: ykducvvcjzdpoojxlsig  (Supabase US test)
--   ⚠️ NO correr esto en producción BR (ejiycfqxgtrzaysgpzmx).
--
-- CÓMO SE USA
--   Pegar este archivo entero en el SQL Editor de Supabase del proyecto US
--   test y ejecutar. Es idempotente: se puede correr dos veces sin romper
--   ni duplicar nada.
--
-- QUÉ APLICA
--   20260822000001_templates_unique_name_por_empresa.sql
--   20260822000002_seed_plantillas_tipos_venta.sql
--
-- POR QUÉ POR SCRIPT Y NO POR `db push`
--   El historial de migraciones de test está divergente del repo (hay 12
--   versiones del repo sin registrar, y 5 en test que el repo no tiene),
--   así que un `db push` intentaría re-aplicar migraciones ya aplicadas.
--   Mismo patrón que sql/aplicar-pendientes-test-PASO1.sql y PASO2.sql.
--
-- AL FINAL corre la verificación: deben salir 4 filas en OK.
-- =====================================================================
...
VALUES ('20260822000001'), ('20260822000002')
ON CONFLICT (version) DO NOTHING;


-- ####################################################################
-- ### Verificación — deben salir 4 filas, todas en OK
-- ####################################################################
SELECT '20260822000001' AS migracion,
       'índice uq_templates_company_name' AS chequeo,
       CASE WHEN EXISTS (SELECT 1 FROM pg_indexes
                          WHERE schemaname='public' AND indexname='uq_templates_company_name')
            THEN 'OK' ELSE 'FALTA' END AS estado
UNION ALL
SELECT '20260822000002',
       'plantillas sembradas (3 por empresa)',
       CASE WHEN (SELECT count(*) FROM public.templates
                   WHERE template_type IN ('anexo_incorporacion','solicitud_cambio_plan','anexo_vigencia_inmediata'))
                 = (SELECT count(*) * 3 FROM public.companies)
            THEN 'OK' ELSE 'FALTA' END
UNION ALL
SELECT '20260822000002',
       'las 3 tienen contenido interpolable',
       CASE WHEN NOT EXISTS (SELECT 1 FROM public.templates
                              WHERE template_type IN ('anexo_incorporacion','solicitud_cambio_plan','anexo_vigencia_inmediata')
                                AND (content IS NULL OR content NOT LIKE '%{{%'))
            THEN 'OK' ELSE 'FALTA' END
UNION ALL
SELECT '20260822000002',
       'las 3 llevan bloque de firma (entran al circuito de firma)',
       CASE WHEN NOT EXISTS (SELECT 1 FROM public.templates
                              WHERE template_type IN ('anexo_incorporacion','solicitud_cambio_plan','anexo_vigencia_inmediata')
                                AND content NOT LIKE '%{{firma_contratante}}%')
            THEN 'OK' ELSE 'FALTA' END
ORDER BY 1, 2;
